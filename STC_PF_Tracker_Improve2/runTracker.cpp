...
int main(int argc, char * argv[])
{
	...
	//视频总帧数
	int mFrameCount = (int)mVideo.get(CV_CAP_PROP_FRAME_COUNT);
	...
	Tracker mTracker;
	bool state = mTracker.init(mFrame, mGrayFrame, mTarget, mFrameCount);

	out << mTarget.x - mTarget.wid << "," << mTarget.y - mTarget.hei << "," << 2 * mTarget.wid << "," << 2 * mTarget.hei << endl;
	...
	while (true)
	{
		...
		mTracker.track(mFrame, mGrayFrame, mTarget);

		mBuf.str("");
		mBuf << mFrameNum;

		putText(mFrame, mBuf.str(), Point(20, 30), FONT_HERSHEY_SIMPLEX, 1, Scalar(0, 0, 255), 3);
		rectangle(mFrame, Rect(mTarget.x - mTarget.wid, mTarget.y - mTarget.hei, 2 * mTarget.wid, 2 * mTarget.hei), Scalar(0, 0, 255), 2);
		imshow("Tracker", mFrame);
		out << mTarget.x - mTarget.wid << "," << mTarget.y - mTarget.hei << "," << 2 * mTarget.wid << "," << 2 * mTarget.hei << endl;
...
double calculateFPS(int mIndex)
{
	...
	Tracker mTracker;
	mTracker.init(mFrame, mGrayFrame, mTarget, (int)mVideo.get(CV_CAP_PROP_FRAME_COUNT));
